...
func TestJavaScriptCoverageFiltering(t *testing.T) {
	tests := []struct {
		...
		// Basic application script
		{
			...
			options:      getDefaultFilterOptions(),
			expectApp:    true,
		...
		// Development mode - more permissive
		{
			...
			source:       "window.__REACT_DEVTOOLS_GLOBAL_HOOK__ = {};",
			options:      getDevelopmentFilterOptions(),
			expectApp:    true,
			expectReason: "application_script",
		},
		...
		// Production mode - strict filtering
		{
			...
			options:      getProductionFilterOptions(),
			expectApp:    false,
			expectReason: "too_small",
		},
...
func TestFilteringStatistics(t *testing.T) {
	...
	applicationIndices, stats := filterApplicationScriptsWithStats(scripts, sources, getDefaultFilterOptions())
